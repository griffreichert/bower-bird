#!/usr/bin/env bash
# Generate the bower-bird digest via Claude Code (subscription, Tier 2).
#
# This is the whole-graph synthesis pass — deliberately run by hand (or from a
# session), NOT crond: driving the subscription from an unattended loop is a
# ToS gray area and burns usage caps. The cheap automated work is the Tier-1
# Haiku drain (`make drain`); this is the expensive, in-the-loop thinking.
#
# First run: do it interactively once (`cd <vault> && claude`) so you can grant
# write permission to the folder; after that this wrapper is a one-shot.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"

VAULT="$(cd "$REPO_ROOT" && uv run python -c \
  'from bower_bird.config import load_config; print(load_config().vault_path)' \
  | tail -1)"

if [ ! -d "$VAULT" ]; then
  echo "Vault folder not found: $VAULT" >&2
  exit 1
fi

cd "$VAULT"
exec claude -p \
  "Generate today's digest following BowerBird/CLAUDE.md. Read the most recent prior digest's '## Feedback' section first and let it steer this run."
